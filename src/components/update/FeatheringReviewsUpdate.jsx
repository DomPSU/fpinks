...
class FeatheringReviewsUpdate extends Component {
  constructor(props) {
    super(props);
    ...
    this.getFeatherngReview = this.getFeatheringReview.bind(this);
  ...
  componentDidMount() {
    ...
    // if url contains query string, get Feathering Review
    if (getURL.indexOf('?') !== -1) {
      this.getFeatheringReview(getURL);
    }
  }

  getFeatheringReview(url) {
    const config = {
      headers: {
        ...
        if (id === 'userID' || id === 'writingSampleID') {
          this.setState({ disableForm: true }, () => {
            this.getFeatheringReview(
              `/feathering-reviews/admin/?FeatheringReviews.user_id=${userID}&FeatheringReviews.writing_sample_id=${writingSampleID}`,
            );
          });
        }
      },
  ...
  handleSubmit(e) {
    ...
    API.instance
      .put(
        `/feathering-reviews/edit/?user_id=${userID}&writing_sample_id=${writingSampleID}`,
        {
          userID,
          writingSampleID,
          approved,
  ...
  render() {
    ...
    return (
      <div className="container text-center">
        <h1 className="p-5">Update a Feathering Review</h1>
        <form>
          <div className="row">
...
export default FeatheringReviewsUpdate;
